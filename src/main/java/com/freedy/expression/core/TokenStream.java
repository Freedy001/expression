...
public class TokenStream {
    private static final Set<String> doubleOps = new HashSet<>(Arrays.asList("||", "&&", "!=", "==", ">=", "<=", "++", "--", "+=", "-=", "/=", "*=", "^=", "&=", "|=", "<<", ">>", ">>>"));
    private static final Set<String> permitOps = new HashSet<>(Arrays.asList("=!", "||!", "&&!", "==!", ">++", "<++", "++<", "++>", "=++", "++=", ">--", "<--", "-->", "--<", "=--", "--=", "+-", "-+", "++-", "--+"));
    private static final Set<String> single2TokenOps = new HashSet<>(Arrays.asList("+++", "---"));
    private static final Set<String> singleOps = new HashSet<>(Arrays.asList("++", "--", "!"));
    ...
    private Function<List<List<Token>>, List<List<Token>>> sorter = lists->{
        ...
        for (List<Token> stream : lists) {
            if ((stream.size() == 1 && stream.get(0) instanceof DefToken && (DefToken)stream.get(0) != null && ((DefToken)stream.get(0)).isFunc()) ||
                    (stream.size() == 1 && stream.get(0) instanceof DirectAccessToken && ((DirectAccessToken)stream.get(0)) != null && "func".equals( ((DirectAccessToken)stream.get(0)).getMethodName()))) {
                funcList.add(stream);
            } else {
                normList.add(stream);
            }
    ...
    public List<Token> getAllTokens() {
        return blockStream.stream().flatMap(Collection::stream).collect(Collectors.toList());
    }
