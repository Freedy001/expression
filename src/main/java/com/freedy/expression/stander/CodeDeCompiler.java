...
public class CodeDeCompiler {
    ...
    public static final Set<Character> splitSet = new HashSet<>(Arrays.asList(' ', '\n', '.', '<', '>', ';', '=', '(', ')', '{', '}'));
    @Getter
    private static final Set<String> keywords = new HashSet<>(Arrays.asList("abstract", "assert", "boolean", "break", "byte", "case", "catch", "char", "class", "const", "continue", "default", "do", "double", "else", "enum", "extends", "false", "final", "finally", "float", "for", "goto", "if", "implements", "import", "instanceof", "int", "interface", "long", "native", "new", "null", "package", "private", "protected", "public", "return", "short", "static", "strictfp", "super", "switch", "synchronized", "this", "throw", "throws", "transient", "try", "true", "void", "volatile", "while", "", "permits"));
    ...
    private static String getCode(List<ClassByteCode> classNameAndByteCode, boolean raw, String method) {
        ...
        map.put("decodestringswitch", "false");
        map.put("", "true");
        ...
        driver.analyse(classNameAndByteCode.stream().map(ClassByteCode::getClassName).collect(Collectors.toList()));
    ...
    private static List<ClassByteCode> getFiles(Class<?> clazz) {
        ...
        if (url.getProtocol().equals("jar") && url.openConnection() instanceof JarURLConnection) {
            ...
            while (entries.hasMoreElements()) {
                ...
                if (name.equals(fullName + ".class") || name.startsWith(fullName + "$")) {
                    ...
                    codeList.add(new ClassByteCode(name, JavaAdapter.readAllBytes(inputStream)));
                }
            }
        } else {
    ...
    private static byte[] getBytecode(Class<?> clazz, String fileName) {
        return JavaAdapter.readAllBytes(Objects.requireNonNull(clazz.getResourceAsStream(fileName)));
    }
...
case "(" : case  ")" :  builder.append(SMALL_BRACKET).append(symbolHighLight).append(END); break;
case "{" : case  " break; }" :  builder.append(BIG_BRACKET).append(symbolHighLight).append(END);
case "<" : case  ">" :  builder.append(NUMBER).append(symbolHighLight).append(END); break;
