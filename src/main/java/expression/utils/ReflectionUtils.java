...
public class ReflectionUtils {
    ...
    public static List<Field> getStaticFieldsRecursion(Class clazz) {
        return getFieldsRecursion(clazz).stream().filter(field -> Modifier.isStatic(field.getModifiers())).collect(Collectors.toList());
    }

    public static List<Method> getStaticMethodsRecursion(Class clazz) {
        return getMethodsRecursion(clazz).stream().filter(method -> Modifier.isStatic(method.getModifiers())).collect(Collectors.toList());
    }
    ...
    public static <T extends Annotation, A> List<T> getAnnotationRecursion(Class<A> clazz, Class<T> annotationClass, ElementType... fieldName) {
        ...
        if (fieldName != null) {
            Arrays.stream(fieldName).distinct().collect(Collectors.toList()).forEach(type -> {
                switch (type) {
                    case TYPE:
    ...
    public static <T> T convertType(Object strValue, Class<T> type) {
        ...
        switch (type.getSimpleName()) {
            case "Integer":
            case "int":
                returnValue = Integer.parseInt(strValue.toString());
                break;
            case "Long":
            case "long":
                returnValue = Long.parseLong(strValue.toString());
                break;
            case "Double":
            case "double":
                returnValue = Double.parseDouble(strValue.toString());
                break;
            case "Float":
            case "float":
                returnValue = Float.parseFloat(strValue.toString());
                break;
            case "Boolean":
            case "boolean":
                returnValue = Boolean.parseBoolean(strValue.toString());
                break;
            case "short":
            case "Short":
                returnValue = Short.parseShort(strValue.toString());
                break;
            case "Byte":
            case "byte":
                returnValue = Byte.parseByte(strValue.toString());
                break;
            case "Character":
            case "char":
                returnValue = strValue.toString().charAt(0);
                break;
            case "String":
            case "Object":
                returnValue = strValue.toString();
                break;
            default:
                throw new UnsupportedOperationException("can not convert String to " + type.getName());
        }
    ...
    public static Object invokeMethod(String methodName, Class<?> targetClass, Object target, Object... args) throws Throwable {
        ...
        if (methodName.equals("<init>")) {
            list.addAll(Arrays.asList(targetClass.getDeclaredConstructors()));
        } else {
                ...
                for (; i < count; i++) {
                    ...
                    if (i == count - 1 && isVar && originMethodArgs.isArray()) {
                        ...
                        if (!originMethodArgs.isAssignableFrom(supplyMethodArgs)) {
                            ...
                            if (o != Boolean.FALSE) {
                                args[i] = o;
                            } else if (args[i] instanceof TokenStream && ((TokenStream) args[i]).getMetadata() instanceof LambdaAdapter) {
                                LambdaAdapter adapter = (LambdaAdapter) ((TokenStream) args[i]).getMetadata();
                ...
                if (i == count) {
                    ...
                    if (lambdaIndex.size() > 0) {
                        ...
                        for (Map.Entry<Integer, Class<?>> entry : lambdaIndex.entrySet()) {
                            if (args[entry.getKey()] instanceof TokenStream && ((TokenStream) args[entry.getKey()]).getMetadata() instanceof LambdaAdapter) {
                                LambdaAdapter adapter=(LambdaAdapter)((TokenStream) args[entry.getKey()]).getMetadata();
                    ...
                    try {
                        ...
                        if (method instanceof Constructor) {
                            Constructor m = (Constructor) method;
...
//        if (count == 1) return args;
    ...
    static {
        convertableMap.put("java.lang.Long", new HashSet<>(Arrays.asList("java.lang.Integer", "java.lang.Short", "int", "long", "short")));
        convertableMap.put("java.lang.Integer", new HashSet<>(Arrays.asList("java.lang.Long", "java.lang.Short", "int", "long", "short")));
        convertableMap.put("java.lang.Short", new HashSet<>(Arrays.asList("java.lang.Integer", "java.lang.Long", "int", "long", "short")));
        convertableMap.put("java.lang.Double", new HashSet<>(Arrays.asList("java.lang.Float", "double", "float")));
        convertableMap.put("java.lang.Float", new HashSet<>(Arrays.asList("java.lang.Double", "double", "float")));
    }


    public static boolean isConvertable(Class<?> originMethodArgs, Class<?> supplyMethodArgs) {
        return Optional.ofNullable(convertableMap.get(originMethodArgs.getName())).orElse(new HashSet<>()).contains(supplyMethodArgs.getName());
    }
