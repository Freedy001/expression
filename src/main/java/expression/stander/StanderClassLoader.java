...
public class StanderClassLoader extends URLClassLoader {
    ...
    private Path getPomPath(String groupId, String artifactId, String versionId, String relevantPath) {
        Path rel, pomPath = Paths.get(repositoryPath).resolve(groupId.replace(".", "\\") + "\\" + artifactId + "\\" + versionId + "\\");
    ...
    private Path getPomPath(InputLocationTracker model) {
        if (model instanceof Dependency) {
            Dependency a = (Dependency) model;
            return getPomPath(a.getGroupId(), a.getArtifactId(), a.getVersion(), null);
        }
        if (model instanceof Parent) {
            Parent a = (Parent) model;
