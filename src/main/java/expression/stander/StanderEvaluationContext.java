...
public class StanderEvaluationContext extends PureEvaluationContext {
    ...
    public StanderEvaluationContext importPack(String pack) {
        importMap.put("package:" + pack.trim(), "*");
        return this;
    }
    ...
    private void init(Set<String> standerFuncPackage) {
        ...
        for (Class<?> aClass : PackageScanner.doScan(standerFuncPackage.toArray(new String[0]), new String[0])) {
                ...
                if (func.enableCMDParameter()) {
                    ...
                    if (types.length > 1) {
                        throw new IllegalArgumentException("?(?*) couldn't enable CMDParameter,because the number of parameter of this function is not 1",
                                ...
                                .map(p -> p.getType().getSimpleName() + " " + p.getName()).collect(Collectors.toList()));
                    }
                    ...
                    for (Field f : ReflectionUtils.getFieldsRecursion(types[0])) {
                        ...
                        paramHelp.append(cmd.value()).append(JavaAdapter.repeat(" ", Math.max(20 - cmd.value().length(), 5))).append(cmd.helpText()).append("\n");
    ...
    @AllArgsConstructor
    @NoArgsConstructor
    public static class FunctionalMethod implements Functional {
        public Object funcObj;
        public Method func;
        public HashMap<String, ValInjector> cmdParamMap;
    ...
    public void registerFunctionWithHelp(String funcName, String help, Functional functional) {
        Method method = functional instanceof FunctionalMethod ? ((FunctionalMethod) functional).func : functional.getClass().getDeclaredMethods()[0];
