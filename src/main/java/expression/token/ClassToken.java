...
public abstract  class ClassToken extends Token implements Assignable
    ...
    protected Object[] getMethodArgs(List<UnsteadyArg> unsteadyArgList) {
        ...
        for (UnsteadyArg arg : unsteadyArgList) {
            switch (arg.getType()) {
case UnsteadyArg.NUMERIC : case  UnsteadyArg.DELAY_TOKEN_STREAM :  args.add(arg.getValue()); break;
 ...
 break;
        ...
        for (String methodArg : argsStr) {
            try {
                ...
                if (matcher.find()) {
                    ...
                    func.setArgName(StringUtils.isEmpty(lambdaArgs) ? new String[0] : Arrays.stream(lambdaArgs.split(",")).map(String::trim).toArray( String[]::new));
                    func.setFuncBody(ts);
     ...
     * 例如:<br/>
     * List a=Arrays.asList(1,2,3,4,5);<br/>
     * ===> a[2]=3;<br/>
     *
